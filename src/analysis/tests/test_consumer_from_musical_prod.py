...
async def test_one_message_to_consumer(db_pool):
    ...
    artists, albums, tracks, liked_artists, liked_tracks = await send_musical_entities(num_iterations=1)
    await asyncio.sleep(5)

    artist = artists[0]
    retrieved_artist = await Artist.get_latest_by_id(artist["id"], pool=db_pool)
    ...
    assert retrieved_artist.created_at.strftime("%Y-%m-%dT%H:%M:%S") == artist["created_at"]
    assert retrieved_artist.updated_at.strftime("%Y-%m-%dT%H:%M:%S") == artist["updated_at"]
    ...
    assert retrieved_track.published_at.strftime("%Y-%m-%dT%H:%M:%S") == track["created_at"]
    assert retrieved_track.updated_at.strftime("%Y-%m-%dT%H:%M:%S") == track["updated_at"]
    ...
    assert retrieved_liked_artist.created_at.strftime("%Y-%m-%dT%H:%M:%S") == liked_artist["created_at"]
    ...
    assert retrieved_liked_track.created_at.strftime("%Y-%m-%dT%H:%M:%S") == liked_track["created_at"]
...
async def test_multiple_messages_to_consumer(db_pool):
    ...
    были ли они успешно вставлены в Database через Kafka Consumer.
    """
    artists, albums, tracks, liked_artists, liked_tracks = await send_musical_entities(num_iterations=5)
    await asyncio.sleep(5)

    for artist in artists:
        retrieved = await Artist.get_latest_by_id(artist["id"], pool=db_pool)
        assert retrieved is not None
        assert retrieved.name == artist["name"]

    for album in albums:
        retrieved = await Album.get_latest_by_id(album["id"], pool=db_pool)
        assert retrieved is not None
        assert retrieved.title == album["title"]

    for track in tracks:
        retrieved = await Track.get_latest_by_id(track["id"], pool=db_pool)
        assert retrieved is not None
        assert retrieved.title == track["title"]

    for liked_artist in liked_artists:
        retrieved = await LikedArtist.get_latest_by_user_artist(
            liked_artist["user_id"], liked_artist["artist_id"], pool=db_pool
        )
        assert retrieved is not None

    for liked_track in liked_tracks:
        retrieved = await LikedTrack.get_latest_by_user_track(
            liked_track["user_id"], liked_track["track_id"], pool=db_pool
        )
        assert retrieved is not None
